...
import {resetPasswordForCompanyUserService} from 'api/authApi'
...
export const WelcomeCompany = ({match}) => {
  //const roles = ['4']
  const history = useHistory()
  const companyId = match.params.companyId.replace(/.*=/, '')
  const userEmail = match.params.email.replace(/.*=/, '')
  const companyName = match.params.contactName.replace(/.*=/, '')
          ...
          >
            <Container>
              ...
              <Box sx={{justifyContent: 'center', display: 'flex', marginRight: 100}}>
                ...
                >
                  <h4 className={classes.forgotPasswordTitle}>Welcome Back</h4>
                  <span>Please, complete your account creation as a company for {companyName}</span>
                </Box>
              </Box>
              <Box sx={{marginLeft: 80, alignItems: 'center', marginRight: 80}}>
                <TextField
                  ...
                  type='email'
                  variant='outlined'
                  value={(inputValue.email = userEmail)}
                  error={isError.email}
