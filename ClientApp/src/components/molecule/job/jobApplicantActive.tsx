...
const JobApplicantTableActive = withRouter(
  ({location, history}: RouteComponentProps<{}, StaticContext, LocationState>) => {
    ...
    const {data, isSuccess, isFetching, refetch} = useGetJobsQuery(
      {
        filter: {...filters, candidateId: CurrentUserCache.userId?.toString(), isDeleted: 'false'},
                  ...
                  return (
                    <TableRow key={index}>
                      ...
                      <TableCell className={classes.tableCell}>
                        <Link
                          onClick={() => {
                            navigator.clipboard.writeText(row.jobOwnerShipEmail)
                            dispatch(
                              setToast({
                        ...
                        >
                          <span style={{cursor: 'pointer', color: '#0091D0'}}>{row.jobOwnerShip}</span>
                        </Link>
                      </TableCell>
                      {isAllowed(CurrentUserCache?.roles, [ModalRoleEnums.Administrator]) && (
                        <TableCell className={classes.tableCell}>
