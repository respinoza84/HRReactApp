...
const JobApplicantTable = withRouter(({location, history}: RouteComponentProps<{}, StaticContext, LocationState>) => {
  ...
  const {data, isSuccess, isFetching, refetch} = useGetJobsQuery(
    {
      filter: {...filters, candidateId: CurrentUserCache.userId?.toString()},
  ...
  // data?.jobs?.items?.filter(
  //   (x) => x.jobExternalType === ('Full Time' || 'Part Time' || 'Multiple Shifts' || 'Contractor' || 'Temp to Hire')
  // )
