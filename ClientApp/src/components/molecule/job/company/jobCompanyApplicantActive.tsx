...
import {Archive, FilterList, Add} from '@material-ui/icons'
import ArchiveModal from '../../delete/archiveModal'
...
const JobApplicantTableActive = withRouter(
  ({location, history}: RouteComponentProps<{}, StaticContext, LocationState>) => {
    ...
    const {data, isSuccess, isFetching, refetch} = useGetJobsQuery(
      {
        filter: {...filters, companyId: CurrentUserCache.companyId?.toString(), isDeleted: 'false'},
          ...
          {!isFetching && items ? (
            <TableHead>
              <TableRow>
                ...
                <TableCell className={classes.headerTableCell}>
                  <span>
                    <Archive fontSize='small' color='inherit' />
                  </span>
                </TableCell>
              </TableRow>
            </TableHead>
                    ...
                    <TableRow key={index}>
                      ...
                      <TableCell className={classes.tableCell}>
                        <Link
                          onClick={() => {
                            window.open(
                              `mailto:${row.jobOwnerShipEmail}?subject=${row.companyName}%20Is%20interested&body=we%20are%20geeting%20to%20assign%20an%20account%20manager%20to%20the%20job`
                            )
                          }}
                        >
                          <span style={{cursor: 'pointer', color: '#0091D0'}}>Request</span>
                        </Link>
                      </TableCell>
...
export {JobApplicantTableActive}
