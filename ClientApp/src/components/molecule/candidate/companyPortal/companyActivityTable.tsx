...
import {useGetCandidateActivitiesQuery} from 'graphql/Candidates/Queries/GetCandidateActivitiesQuery.generated'
...
const CompanyActivityTable = withRouter(({match}: RouteComponentProps<{candidateId: string}>) => {
  ...
  const {data, isFetching} = useGetCandidateActivitiesQuery(
    {
      entityId: parseInt(
  ...
  /*useEffect(() => {
    setTotalRows(data?.documentsById?.totalCount ?? 0)
  }, [data?.documentsById?.totalCount, isSuccess])*/
  ...
  const items = data?.candidateActivities?.items
