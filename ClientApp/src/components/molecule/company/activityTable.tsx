...
import {useGetCompanyActivitiesQuery} from 'graphql/Company/Queries/GetCompanyActivitiesQuery.generated'
...
const ActivityTable = withRouter(({match}: RouteComponentProps<{companyId: string}>) => {
  ...
  const {data, isFetching} = useGetCompanyActivitiesQuery(
    {
  ...
  const items = data?.companyActivities?.items
